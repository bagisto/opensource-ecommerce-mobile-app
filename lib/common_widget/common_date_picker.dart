...
class CommonDatePicker extends StatefulWidget{
  ...
  bool? isRequired;
  int? save;
  CommonDatePicker({Key? key, this.controller, this.save, this.hintText,this.labelText, this.helperText,this.isRequired = false}) : super(key: key);
  @override
...
class _CommonDatePickerState extends State<CommonDatePicker> {
  ...
  Future<void> _selectedDate() async {
    DateTime? picked = await showDatePicker(
        ...
        context: context,
        initialDate: (widget.controller?.text ?? "").isNotEmpty ? selectedDate : (widget.save == 1 ? selectedDate.add(const Duration(days: 0)) : DateTime.now().add(const Duration(days: -1))),
        firstDate: widget.save == 1 ? selectedDate.add(const Duration(days: 0)) : DateTime(1950),
