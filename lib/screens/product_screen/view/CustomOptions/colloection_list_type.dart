...
class CollectionListType extends StatefulWidget {
  ...
  dynamic seletedId;
  List? optionArray;
  CollectionListType({Key? key, this.variation, this.callback,this.options,this.seletedId, this.optionArray}) : super(key: key);
  @override
            ...
            child:  Container(
              decoration: BoxDecoration(
                  ...
                  borderRadius: const BorderRadius.all(
                      Radius.circular(4.0)),
                  border:Border.all(
                    color: set ? Theme.of(context).colorScheme.onPrimary : Colors.grey,
                    width: set ? 2.0 : 2.0, //
                  )

              ),
              padding: const EdgeInsets.fromLTRB(14, 6, 14, 6),
              child: Text(item.label ?? '',style: TextStyle(color:set ? Colors.white:Theme.of(context).colorScheme.onPrimary,fontSize: 15,)),
            )
        );
      }).toList() ?? [],
